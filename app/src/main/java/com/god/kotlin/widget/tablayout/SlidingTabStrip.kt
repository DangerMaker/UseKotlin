...
internal class SlidingTabStrip @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (childCount > 0) {
            ...
            val subheight = selectedTitle.baseline
            var color = tabColorizer?.getIndicatorColor(mSelectedPosition) ?: 0

            if (mSelectionOffset > 0f && mSelectedPosition < getChildCount() - 1) {
                val nextColor = tabColorizer?.getIndicatorColor(mSelectedPosition + 1) ?: 0
                if (color != nextColor) {
            ...
            mSelectedIndicatorPaint.textSize = 50f
            mSelectedIndicatorPaint.style = Paint.Style.STROKE
            mSelectedIndicatorPaint.textAlign = Paint.Align.CENTER
        }
    }
